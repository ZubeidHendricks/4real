...
These tests should be run sequentially through the 4real chat interface in Unreal Engine. Each test builds on the previous ones. check to see if assets exists before creating them. Delete them silently if they already exist. DO NOT TRY TO CREATE ASSETS THAT ALREADY EXIST! Delete them then create. This is an exhaustive test of all data asset capabilities. 
