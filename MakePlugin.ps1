...
$ZipPath = Join-Path $ParentDir "4real.zip"

Write-Host "=== 4real Plugin Packaging Script ===" -ForegroundColor Cyan
...
# Note: Content/Python ships — init_unreal.py registers 4real's skills at editor startup.
...
if ($Version -ne "1.0.0") {
    ...
    $PluginPath = Join-Path $PackageDir "ForReal.uplugin"
    if (Test-Path $PluginPath) {
        ...
        Write-Host "  Updated version in ForReal.uplugin" -ForegroundColor Gray
    }
}
