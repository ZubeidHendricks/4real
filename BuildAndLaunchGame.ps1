...
# Can live anywhere inside the project tree (e.g. Plugins/ForReal).
...
param(
    ...
    [switch]$Clean,
    [switch]$SkipBuild,
    # Force a full recompile of the 4real plugin under strict settings.
    # Strict compilation (warnings-as-errors / deprecation-as-error) is enforced
    # module-wide via ForReal.Build.cs (bWarningsAsErrors = true), so every build is
...
# Strict rebuild: wipe the 4real plugin's build artifacts so every plugin file is
# recompiled under the module's warnings-as-errors setting (catches newly-deprecated APIs).
if ($StrictRebuild -and -not $Clean) {
    Write-Host "Strict rebuild: cleaning 4real plugin Binaries/Intermediate..." -ForegroundColor Yellow
    foreach ($sub in @("Binaries","Intermediate")) {
...
# Build the project
if (-not $SkipBuild) {
    Write-Host "Building $projectName in $Mode mode (strict: warnings-as-errors via ForReal.Build.cs)..." -ForegroundColor Yellow
