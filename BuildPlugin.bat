@echo off
REM 4real Plugin Build Script
REM Universal build script that works with any Unreal Engine project
REM 
REM Usage Scenarios:
REM   1. Place in project's Plugins/ForReal/ folder - builds for that project
...
echo.
echo ====================================
echo 4real Universal Plugin Build Script
echo ====================================
echo.
...
echo Please ensure this script is in the 4real plugin root folder.
echo.
pause
exit /b 1
...
echo For best compatibility, place this script in: YourProject\Plugins\4real\
echo.
...
if %ERRORLEVEL% EQU 0 (
    ...
    echo ====================================
    echo.
    echo 4real plugin compiled successfully.
    
    REM Check if binaries exist in plugin directory
    if exist "%PLUGIN_DIR%\Binaries\Win64\UnrealEditor-ForReal.dll" (
        echo Found: %PLUGIN_DIR%\Binaries\Win64\UnrealEditor-ForReal.dll
        for %%A in ("%PLUGIN_DIR%\Binaries\Win64\UnrealEditor-ForReal.dll") do echo Size: %%~zA bytes
        echo.
