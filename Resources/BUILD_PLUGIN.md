# Building 4real Plugin

This document explains how to build the 4real plugin to resolve the "Missing Modules" error when adding it to a fresh Unreal Engine project.

## Quick Start

**Double-click this file to build:**
```
Plugins/ForReal/BuildPlugin.bat
```
...
When you copy 4real to a fresh Unreal Engine project, you may see this error:
```
Missing Modules
...
This happens because 4real is a **C++ Editor plugin** that requires compilation. The build script compiles the plugin for your specific Unreal Engine version.
...
3. **Builds the 4real plugin** using Unreal's build tools
...
**Solution**: Make sure the plugin is located in `YourProject/Plugins/ForReal/`
...
4real/
  Binaries/
    Win64/
      UnrealEditor-4real.dll    # Main plugin binary
      UnrealEditor-4real.pdb    # Debug symbols
...
2. Copy the entire 4real folder to `MyProject/Plugins/`
