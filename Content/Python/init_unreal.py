...
#
# Registers ForReal's markdown skill packs as native Unreal Engine 5.8 agent skills, so they are
...
def _register_skill(class_suffix, description, instructions):
    ...
    class_name = "ForReal_" + class_suffix
    ...
    _FORREAL_SKILL_CLASSES.append(skill_cls)
...
def _register_forreal_skills():
    if not _SKILLS_DIR.is_dir():
        unreal.log_warning("ForReal: Skills directory not found at {}".format(_SKILLS_DIR))
        return
    ...
    for skill_dir in sorted(p for p in _SKILLS_DIR.iterdir() if p.is_dir()):
        ...
        try:
            ...
            unreal.log_warning("ForReal: failed to register skill '{}': {}".format(skill_name, exc))

    unreal.log("ForReal: registered {} skill(s) with Unreal's AgentSkill system".format(count))


_register_forreal_skills()
