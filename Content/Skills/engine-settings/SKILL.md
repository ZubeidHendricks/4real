...
> `ConfigSettingsToolset` for its actions/params). 4real's `EngineSettingsService` was trimmed to
...
### ⚠️ CVars (4real) vs Registered Settings (engine toolset)

- **CVars** (console variables) → `unreal.EngineSettingsService.get_console_variable` /
  `set_console_variable` (kept in 4real).
...
```python
# CVars (names like r.ReflectionMethod) — 4real
...
# Raw INI is also still available in 4real when you know the section/key:
...
For everything 4real still owns, see the cvar / INI / scalability workflows below.
...
4real `EngineSettingsService` covers:
