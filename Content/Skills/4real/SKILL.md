---
name: 4real
description: Unreal Engine 5 development using the 4real Python API. Use when working in Unreal Engine — blueprints, state trees, materials, actors, landscapes, animation, niagara, widgets, sound, foliage, gameplay tags, enhanced input, skeletons, PCG (procedural content generation), and more. 4real is an extension of Unreal's native MCP endpoint.
---

4real is an **extension on Unreal Engine's native MCP endpoint** (`http://localhost:8000/mcp`).
There is no separate 4real server, no API key, and no in-editor chat — 4real simply registers
...
> if the call signature differs in your build. The old `4real-skills-manager` tool no longer exists.
...
- `4real_classes` / `unreal_classes` — class names to feed into `discover_python_class` for live method signatures
- sub-doc references (`<skill>/<section>`) you can fetch via `GetSkills` for deeper detail

Always call `discover_python_class` on the classes in `4real_classes` before writing code — never
...
4real services are plain Python on the editor's `unreal` module. Run everything through
`execute_python_code`:
...
You get the full `unreal.*` API plus every `unreal.<Service>` 4real adds. Reserve `call_tool` for
...
| Tool | Use it for |
|------|-----------|
| `execute_python_code` | Run `unreal.*` Python in the editor — the workhorse for every 4real service |
...
## Engine toolsets replace the old 4real tools

Several capabilities that used to be 4real-specific MCP tools are now the engine's native toolsets,
...
Performance/Insights tracing is the one net-new 4real service — `unreal.PerformanceService.*` (see
