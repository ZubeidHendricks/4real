...
> 4real's `ProjectSettingsService` was trimmed to the delta the engine doesn't cover:
...
the engine **`ConfigSettingsToolset`**. From 4real you instead address settings by their INI
**section / key / file**:
...
**`ConfigSettingsToolset`** (`call_tool`). Or write the INI keys directly from 4real:
...
`call_tool`). From 4real you can enumerate the raw INI keys of a known section instead:
