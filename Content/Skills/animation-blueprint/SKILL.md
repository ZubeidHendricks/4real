...
> An AnimBP **is a Blueprint**. The state-machine/AnimGraph authoring here is 4real
> `AnimGraphService` (intact). For its **variables and compile**, note 4real's `BlueprintService`
...
#    Variable creation + compile are ENGINE BlueprintTools (call_tool), not 4real:
...
# 2) Draw the transition (4real AnimGraphService)
