...
> The old **`manage_asset` MCP tool is GONE.** 4real's `AssetDiscoveryService` was trimmed to only
...
## Data Assets & Data Tables — no dedicated 4real service (issues #451, #452)
...
| Import image from disk (crash-safe) | `unreal.AssetDiscoveryService.import_asset` / `import_texture` (**stay on 4real — see below**) |
...
4real covers the **single/primary** queries above; the **list-all** and **open** operations live on
...
# "Open the selected asset" = 4real primary-selection + Epic OpenEditorForAsset
...
`Saved/Screenshots` (and `Saved/4real/Screenshots`):
...
### Import / Export Textures (4real — crash-safe)
...
### Editor State & Content Browser (4real)
...
Assets not covered by a 4real service (e.g., `LandscapeGrassType`) require `AssetToolsHelpers` + a
