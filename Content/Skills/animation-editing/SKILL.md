...
> `AgentSkillToolset` (`ListSkills` / `GetSkills`) — there is no `4real-skills-manager` tool. Run the
> 4real services below with `execute_python_code` (`unreal.<Service>.<method>()`).
