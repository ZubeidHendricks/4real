...
| `add_node()` / `add_function_call_node()` / `add_event_node()` | **batch:** 4real `build_graph()`; **single node:** engine `BlueprintTools.create_node` via `call_tool`; **editor-style spawner:** `create_node_by_key()` |
...
> 4real's own `unreal.BlueprintService.*` methods are unaffected — they take plain string paths.
...
>
> 1. **Batch (preferred for 2+ nodes):** `unreal.BlueprintService.build_graph(...)` — the 4real
...
> the timeline + custom-event-input CRUD, `build_graph`, `auto_layout_*`, etc.) are the 4real
...
# Now the getter resolves — 4real build_graph
...
Current UE 5.7 / 4real graph details for this workflow:
...
This skill index contains the frontmatter, intro, and the **Critical Rules / gotchas** section. For everything else, load the matching sub-doc by reading its file under this same folder (`Plugins/ForReal/Content/Skills/blueprint-graphs/`):
