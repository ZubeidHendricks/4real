...
> **Architecture (read first):** 4real extends Unreal's native MCP endpoint. Emitter **module and
...
> through `call_tool`. 4real's `NiagaraEmitterService` was trimmed to the **color/curve authoring**
> the engine doesn't provide, plus a rapid-iteration-parameter reader.

**4real `NiagaraEmitterService` (this skill) keeps ONLY color/curve authoring + a param reader:**
...
> For building an emitter entirely from a **scratch-pad / Custom HLSL** module (which IS a 4real
...
| Task | Sub-doc / where |
|------|-----------------|
| Recolor an emitter (hue shift / tint / scale) | `color.md` (4real) |
| Manage ColorFromCurve modules / curve keys | `color.md` (4real) |
| Read rapid-iteration params | `reference.md` (4real) |
...
| Build an emitter from scratch / Custom-HLSL scratch module | `scratch-pad.md` (4real) |
...
- **`reference.md`** — 4real color/curve + rapid-iteration-param readers, plus where module/renderer
