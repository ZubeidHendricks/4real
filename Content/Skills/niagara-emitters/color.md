...
> `call_tool`. The rapid-iteration-param writes below stay on 4real `NiagaraService`. Prefer
...
# set Scale Color directly (rapid-iteration write stays on 4real NiagaraService):
unreal.NiagaraService.set_rapid_iteration_param(
    system_path, emitter_name,
    f"Constants.{emitter_name}.Color.Scale Color",
    "(0.0, 2.0, 0.0)"
...
To **keep the curve animation but tint it** — note `set_color_tint` (4real) already does exactly
