...
description: Niagara emitter quick reference — 4real rapid-iteration-parameter readers/writers, where module/renderer CRUD lives (engine NiagaraToolsets), and common property names.
---
...
| **Read** rapid-iteration params | 4real `NiagaraService.list_rapid_iteration_params` / `NiagaraEmitterService.get_rapid_iteration_parameters` |
| **Write** rapid-iteration params | 4real `NiagaraService.set_rapid_iteration_param[_by_stage]` |
| Color / curve authoring | 4real `NiagaraEmitterService` (see `color.md`) |
| Scratch-pad / Custom HLSL graphs | 4real `NiagaraScratchPadService` (see `scratch-pad.md`) |
...
## Rapid-iteration parameters (4real)
