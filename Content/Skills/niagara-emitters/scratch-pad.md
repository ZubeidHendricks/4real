...
Once the emitter + modules exist, configure spawn rate with the 4real rapid-iteration write. Use the
...
The rest of THIS doc — the scratch-pad / Custom HLSL graph authoring — is the 4real
...
| Build per-particle math / splat logic in HLSL | **`NiagaraScratchPadService`** (4real) |
| Wire a Custom HLSL node with typed inputs/outputs | **`NiagaraScratchPadService`** (4real) |
| Add Map Get / Map Set / Op / If nodes | **`NiagaraScratchPadService`** (4real) |
