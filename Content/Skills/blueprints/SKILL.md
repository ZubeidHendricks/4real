...
`BlueprintTools` `call_tool` path — 4real's own `unreal.BlueprintService.*` Python methods still take
plain string paths.
...
Blueprint basics now live on the engine's `BlueprintTools` toolset (4real extends the native MCP
...
Refining the graph afterward still uses the surviving 4real delta methods
