...
Unreal 5.8's native **`GameplayTagsToolset`** (call it via `call_tool`). 4real keeps only the
...
> 4real's `add_tags` remains for registering several tags in one call.
...
if unreal.GameplayTagService.has_tag(tag_name):
    ...
    # Register it (bulk-capable 4real helper; or use the engine GameplayTagsToolset)
...
4real delta (call as `unreal.GameplayTagService.<method>` via `execute_python_code`):
