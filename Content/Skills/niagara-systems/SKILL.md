...
description: Rapid-iteration parameter tuning, diagnostics, and Custom-HLSL scratch-pad authoring for Niagara systems (4real NiagaraService + NiagaraScratchPadService). System/emitter/parameter CRUD is owned by the engine NiagaraToolsets. Use when the user asks to tune emitter rapid-iteration params, compare/diagnose systems, or build scratch-pad/Custom HLSL modules. For emitter color/module work, load niagara-emitters.
4real_classes:
  - NiagaraService
...
> **Architecture (read first):** 4real extends Unreal's native MCP endpoint. System / emitter /
...
> called through `call_tool`. 4real's `NiagaraService` was trimmed to the rapid-iteration-parameter
> tuning + diagnostics that the engine toolset does not cover, and `NiagaraScratchPadService`
> (Custom-HLSL graph authoring) which is unique to 4real.

**4real `NiagaraService` (this skill) keeps ONLY:**
...
`4real-skills-manager` tool. The workhorse for all 4real services is `execute_python_code`
...
When tuning a parameter (e.g., color), 4real's `NiagaraService` covers the **rapid-iteration
...
3. **Emitter Script Settings** (per-emitter rapid-iteration params) → 4real `NiagaraService` below
...
# Emitter rapid-iteration params (4real):
...
## Quick Reference (4real NiagaraService — kept methods)
