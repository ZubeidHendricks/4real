...
namespace MapBlockoutImage
{
	...
	namespace Colors
	{
		extern FORREAL_API const FColor MainRoad;        // White
		extern FORREAL_API const FColor DirtRoad;        // Brown
		extern FORREAL_API const FColor Treeline;        // Light Purple
		extern FORREAL_API const FColor Forest;          // Dark Purple
		extern FORREAL_API const FColor Building;        // Light Grey
		extern FORREAL_API const FColor Bridge;          // Orange
		extern FORREAL_API const FColor Railway;         // Dark Grey
		extern FORREAL_API const FColor POIBoundary;     // Bright Green
		extern FORREAL_API const FColor Field;           // Yellow
		extern FORREAL_API const FColor Scrub;           // Light Blue
		extern FORREAL_API const FColor River;           // Blue (context only)
		extern FORREAL_API const FColor Background;      // Dark
		extern FORREAL_API const FColor Panel;           // Color-key panel
		extern FORREAL_API const FColor Grid;            // Faint reference grid
		extern FORREAL_API const FColor Ink;             // Text
	}

	/** Allocate an RGBA8 bitmap filled with a single color. */
	FORREAL_API void NewBitmap(
		TArray<FColor>& OutBitmap, int32 Width, int32 Height, FColor Fill);

	/** Bresenham line into an RGBA8 bitmap. */
	FORREAL_API void DrawLine(
		TArray<FColor>& Bitmap, int32 W, int32 H,
		int32 X0, int32 Y0, int32 X1, int32 Y1,
		FColor Color, int32 ThicknessRadius = 0);

	FORREAL_API void DrawPolyline(
		TArray<FColor>& Bitmap, int32 W, int32 H,
		const TArray<FIntPoint>& Points,
		FColor Color, int32 ThicknessRadius = 0);

	/** Filled disk. */
	FORREAL_API void DrawDisk(
		TArray<FColor>& Bitmap, int32 W, int32 H,
		int32 Cx, int32 Cy, int32 Radius, FColor Color);

	/** Outlined circle (used for POI boundary). */
	FORREAL_API void DrawCircleOutline(
		TArray<FColor>& Bitmap, int32 W, int32 H,
		int32 Cx, int32 Cy, int32 Radius, FColor Color, int32 Thickness = 2);

	/** Filled axis-aligned rectangle. */
	FORREAL_API void DrawRect(
		TArray<FColor>& Bitmap, int32 W, int32 H,
	 ...
	 */
	FORREAL_API void RenderHeatmap(
	 ...
	 */
	FORREAL_API int32 DrawColorKey(
		TArray<FColor>& Bitmap, int32 W, int32 H,
	 ...
	 * test fixtures.
	 */
	FORREAL_API bool LoadGrayscalePNG(
