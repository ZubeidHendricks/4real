...
namespace MapBlockoutMath
{
	...
	FORREAL_API void ResampleBilinear(
	...
	FORREAL_API void GaussianBlur(
		TArray<float>& InOut, int32 W, int32 H, float Sigma);

	/** Vertical flip (north <-> south). Used to translate landcover grid row order. */
	FORREAL_API void FlipVertical(TArray<float>& InOut, int32 W, int32 H);
	FORREAL_API void FlipVerticalU8(TArray<uint8>& InOut, int32 W, int32 H);
	 ...
	 */
	FORREAL_API void ComponentSizes(
	 ...
	 */
	FORREAL_API void GenerateNoiseField(
	 ...
	 */
	FORREAL_API void DistanceTransformEDT(
	 ...
	 */
	FORREAL_API int32 LabelConnectedComponents(
		const TArray<uint8>& Mask, int32 W, int32 H,
	...
	FORREAL_API void RasterizeLine(
	...
	FORREAL_API void RasterizePolygonFilled(
		TArray<uint8>& Mask, int32 W, int32 H,
