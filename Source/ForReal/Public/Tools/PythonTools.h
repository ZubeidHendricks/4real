...
// Forward declarations - in ForReal namespace
namespace ForReal
{
...
class FORREAL_API UPythonTools : public UObject
{
	GENERATED_BODY()
...
public:
	 ...
	 */
	UFUNCTION(BlueprintCallable, Category="ForReal|Python", meta=(
	...
	static FString ConvertModuleInfoToJson(const ForReal::FPythonModuleInfo& Info);
	static FString ConvertClassInfoToJson(const ForReal::FPythonClassInfo& Info);
	static FString ConvertFunctionInfoToJson(const ForReal::FPythonFunctionInfo& Info);
	static FString ConvertExecutionResultToJson(const ForReal::FPythonExecutionResult& Result);

	// Helper to get or create service instances
	static TSharedPtr<ForReal::FPythonDiscoveryService> GetDiscoveryService();
...
private:
	static TSharedPtr<ForReal::FPythonExecutionService> GetExecutionService();
	static TSharedPtr<ForReal::FPythonSchemaService> GetSchemaService();
};
