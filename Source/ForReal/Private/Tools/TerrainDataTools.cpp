...
// Calls forreal.com terrain API endpoints authenticated with the user's ForReal API key.
...
#include "Settings/ForRealEditorSettings.h"
...
static FString GetForRealApiKey()
{
	// Primary: Editor Preferences > Plugins > ForReal (UForRealEditorSettings::ApiKey).
	if (const UForRealEditorSettings* Settings = GetDefault<UForRealEditorSettings>())
	{
		if (!Settings->ApiKey.IsEmpty())
			return Settings->ApiKey;
	}

	// Backward-compat: the legacy [ForReal] ForRealApiKey key written by older builds.
	FString Key;
	GConfig->GetString(TEXT("ForReal"), TEXT("ForRealApiKey"), Key, GEditorPerProjectIni);
	return Key;
}

static FString GetTerrainBaseUrl()
{
	FString Url = TEXT("https://www.forreal.com");
	GConfig->GetString(TEXT("ForReal.Terrain"), TEXT("ApiBaseUrl"), Url, GEngineIni);
	return Url;
}
...
//
// Assumptions (standard ForReal landscape import workflow):
...
{
	const FString ApiKey = GetForRealApiKey();
	if (ApiKey.IsEmpty())
		return BuildErrorJson(TEXT("NO_API_KEY"), TEXT("No ForReal API key configured. Set it in Editor Preferences > Plugins > ForReal (get a free key at https://www.forreal.com/login)."));
...
static FString ActionPreviewElevation(const TMap<FString, FString>& Params)
{
	const FString ApiKey = GetForRealApiKey();
	if (ApiKey.IsEmpty())
		return BuildErrorJson(TEXT("NO_API_KEY"), TEXT("No ForReal API key configured. Set it in Editor Preferences > Plugins > ForReal."));
