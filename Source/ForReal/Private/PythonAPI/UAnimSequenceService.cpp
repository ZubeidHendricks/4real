	...
	FAnimationPoseCaptureResult& OutResult)
{
	...
	// Use ForReal screenshots directory as default if no path provided
	FString ActualOutputPath = OutputPath;
	if (ActualOutputPath.IsEmpty())
	{
		FString ScreenshotsDir = FForRealPaths::GetScreenshotsDir();
	...
	int32 ImageHeight)
{
	...
	// Use ForReal screenshots directory as default if no directory provided
	FString ActualOutputDir = OutputDirectory;
	if (ActualOutputDir.IsEmpty())
	{
		FString AnimName = FPaths::GetBaseFilename(AnimPath);
		ActualOutputDir = FForRealPaths::GetScreenshotsDir() / AnimName;
	}
