...
DEFINE_LOG_CATEGORY_STATIC(LogForRealUVMapping, Log, All);
...
FUVMappingResult UUVMappingService::MakeFailure(const FString& MeshPath, const FString& Message)
{
	...
	UE_LOG(LogForRealUVMapping, Warning, TEXT("UVMappingService failure on %s: %s"), *MeshPath, *Message);
	return Result;
}
...
TArray<FUVChannelInfo> UUVMappingService::ListUVChannels(const FString& MeshPath)
{
	...
	UE_LOG(LogForRealUVMapping, Warning, TEXT("ListUVChannels: %s is not a StaticMesh or SkeletalMesh"), *MeshPath);
	return Result;
}
	...
	FUVChannelInfo& OutInfo)
{
	...
	if (!StaticMesh)
	{
		UE_LOG(LogForRealUVMapping, Warning, TEXT("GetUVChannelInfo: only StaticMesh supports detailed stats. Path: %s"), *MeshPath);
		return false;
	}
