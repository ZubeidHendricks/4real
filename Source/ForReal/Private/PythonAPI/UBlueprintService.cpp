	...
	const FString& DispatcherName)
{
	...
	const FScopedTransaction Transaction(NSLOCTEXT("ForReal", "AddEventDispatcher", "Add Event Dispatcher"));
	Blueprint->Modify();
	...
	const FString& DispatcherName)
{
	...
	const FScopedTransaction Transaction(NSLOCTEXT("ForReal", "RemoveEventDispatcher", "Remove Event Dispatcher"));
	Blueprint->Modify();
	...
	const FString& ContainerType)
{
	...
	const FScopedTransaction Transaction(NSLOCTEXT("ForReal", "AddEventDispatcherParam", "Add Event Dispatcher Parameter"));
	Blueprint->Modify();
...
static bool ForReal_HasUserDefinedPin(const UK2Node_EditablePinBase* Node, const FName PinName)
{
	if (!Node)
	{
		return false;
	...
	const FString& ContainerType)
{
	...
	if (ForReal_HasUserDefinedPin(CustomEvent, FName(*ParameterName)))
	{
	...
	const FString& ContainerType)
{
	...
	if (bWantsRename && ForReal_HasUserDefinedPin(CustomEvent, FName(*NewName)))
	{
...
static UTimelineTemplate* ForReal_ResolveTimeline(UBlueprint* Blueprint, const FString& TimelineName, FString& OutError)
{
	if (!Blueprint)
	{
...
static FTTFloatTrack* ForReal_FindFloatTrack(UTimelineTemplate* Template, const FString& TrackName)
{
	if (!Template)
	{
		return nullptr;
...
static FTTTrackBase* ForReal_FindAnyTrack(UTimelineTemplate* Template, const FName Name, FTTTrackBase::ETrackType& OutType, int32& OutIndex)
{
	if (!Template)
	{
		return nullptr;
...
static void ForReal_TrackCurves(FTTTrackBase* Track, FTTTrackBase::ETrackType Type, TArray<FRichCurve*>& OutCurves)
{
	if (!Track)
	{
		return;
...
static void ForReal_TrackCurveObjects(FTTTrackBase* Track, FTTTrackBase::ETrackType Type, TArray<UCurveBase*>& OutCurves)
{
	if (!Track)
	{
		return;
...
static void ForReal_ParseInterp(const FString& InMode, ERichCurveInterpMode& OutInterp, ERichCurveTangentMode& OutTangent)
{
...
static void ForReal_AddCurveKey(FRichCurve& Curve, float Time, float Value, ERichCurveInterpMode Interp, ERichCurveTangentMode Tangent)
{
...
static int32 ForReal_FindDisplayIndex(UTimelineTemplate* Template, FTTTrackBase::ETrackType Type, int32 TrackIndex)
{
	...
	const FString& TrackName)
{
	...
	FString Error;
	UTimelineTemplate* Template = ForReal_ResolveTimeline(Blueprint, TimelineName, Error);
	if (!Template)
	{
	...
	const FString& InterpMode)
{
	...
	FTTFloatTrack* Track = ForReal_FindFloatTrack(Template, TrackName);
...
static UTimelineTemplate* ForReal_PrepareTrackAdd(const TCHAR* FnName, UBlueprint* Blueprint, const FString& TimelineName, const FString& TrackName,
...
static void ForReal_FinishTrackAdd(UBlueprint* Blueprint, UTimelineTemplate* Template, UK2Node_Timeline* Node, FTTTrackBase::ETrackType Type, int32 NewIndex)
{
	FTTTrackId NewTrackId;
...
bool UBlueprintService::AddTimelineVectorTrack(const FString& BlueprintPath, const FString& TimelineName, const FString& TrackName)
{
	...
	UTimelineTemplate* Template = ForReal_PrepareTrackAdd(TEXT("AddTimelineVectorTrack"), Blueprint, TimelineName, TrackName, TrackFName, Node);
	...
	ForReal_FinishTrackAdd(Blueprint, Template, Node, FTTTrackBase::TT_VectorInterp, NewIndex);
...
bool UBlueprintService::AddTimelineColorTrack(const FString& BlueprintPath, const FString& TimelineName, const FString& TrackName)
{
	...
	FName TrackFName; UK2Node_Timeline* Node = nullptr;
	UTimelineTemplate* Template = ForReal_PrepareTrackAdd(TEXT("AddTimelineColorTrack"), Blueprint, TimelineName, TrackName, TrackFName, Node);
	if (!Template) { return false; }
	Node->Modify(); Template->Modify();
	...
	ForReal_FinishTrackAdd(Blueprint, Template, Node, FTTTrackBase::TT_LinearColorInterp, NewIndex);
...
bool UBlueprintService::AddTimelineEventTrack(const FString& BlueprintPath, const FString& TimelineName, const FString& TrackName)
{
	...
	FName TrackFName; UK2Node_Timeline* Node = nullptr;
	UTimelineTemplate* Template = ForReal_PrepareTrackAdd(TEXT("AddTimelineEventTrack"), Blueprint, TimelineName, TrackName, TrackFName, Node);
	if (!Template) { return false; }
	Node->Modify(); Template->Modify();
	...
	ForReal_FinishTrackAdd(Blueprint, Template, Node, FTTTrackBase::TT_Event, NewIndex);
...
bool UBlueprintService::RemoveTimelineTrack(const FString& BlueprintPath, const FString& TimelineName, const FString& TrackName)
{
	...
	if (!ForReal_FindAnyTrack(Template, FName(*TrackName), Type, Index))
	{
	...
	const int32 DisplayIdx = ForReal_FindDisplayIndex(Template, Type, Index);
...
bool UBlueprintService::RenameTimelineTrack(const FString& BlueprintPath, const FString& TimelineName, const FString& OldTrackName, const FString& NewTrackName)
{
	...
	FTTTrackBase::ETrackType Type; int32 Index;
	FTTTrackBase* Track = ForReal_FindAnyTrack(Template, OldName, Type, Index);
	if (!Track)
	{
...
bool UBlueprintService::AddTimelineVectorKey(const FString& BlueprintPath, const FString& TimelineName, const FString& TrackName, float Time, float X, float Y, float Z, const FString& InterpMode)
{
	...
	FTTTrackBase::ETrackType Type; int32 Index;
	FTTTrackBase* Track = ForReal_FindAnyTrack(Template, FName(*TrackName), Type, Index);
	...
	ERichCurveInterpMode I; ERichCurveTangentMode Tn; ForReal_ParseInterp(InterpMode, I, Tn);
	Curve->Modify();
	const float V[3] = { X, Y, Z };
	for (int32 i = 0; i < 3; ++i) { ForReal_AddCurveKey(Curve->FloatCurves[i], Time, V[i], I, Tn); }
	FBlueprintEditorUtils::MarkBlueprintAsModified(Blueprint);
...
bool UBlueprintService::AddTimelineColorKey(const FString& BlueprintPath, const FString& TimelineName, const FString& TrackName, float Time, float R, float G, float B, float A, const FString& InterpMode)
{
	...
	for (int32 i = 0; i < 4; ++i) { ForReal_AddCurveKey(Curve->FloatCurves[i], Time, V[i], I, Tn); }
	FBlueprintEditorUtils::MarkBlueprintAsModified(Blueprint);
...
bool UBlueprintService::AddTimelineEventKey(const FString& BlueprintPath, const FString& TimelineName, const FString& TrackName, float Time)
{
	...
	ForReal_AddCurveKey(Curve->FloatCurve, Time, 1.0f, RCIM_Constant, RCTM_Auto);
	FBlueprintEditorUtils::MarkBlueprintAsModified(Blueprint);
...
bool UBlueprintService::RemoveTimelineKey(const FString& BlueprintPath, const FString& TimelineName, const FString& TrackName, float Time, float Tolerance)
{
	...
	TArray<UCurveBase*> CurveObjs; ForReal_TrackCurveObjects(Track, Type, CurveObjs);
	TArray<FRichCurve*> Curves; ForReal_TrackCurves(Track, Type, Curves);
	...
	float R, float G, float B, float A)
{
	...
	const FScopedTransaction Transaction(NSLOCTEXT("ForReal", "AddCommentNode", "Add Comment Node"));
	Graph->Modify();
	...
	float R, float G, float B, float A)
{
	...
	const FScopedTransaction Transaction(NSLOCTEXT("ForReal", "AddCommentAroundNodes", "Add Comment Around Nodes"));
	Graph->Modify();
...
FBlueprintFocusContext UBlueprintService::GetFocusedGraphContext()
{
	...
	// assistant's own docked widget; ForReal is an external MCP agent with no
	...
	FString& OutError)
{
	...
	const int32 NumChains = ForRealLayout::LayeredLayout(LayoutNodes, 100.0f, 100.0f);
	FBlueprintEditorUtils::MarkBlueprintAsModified(Blueprint);
	...
	FString& OutError)
{
	...
	const int32 NumChains = ForRealLayout::LayeredLayout(LayoutNodes, OriginX, OriginY);
	FBlueprintEditorUtils::MarkBlueprintAsModified(Blueprint);
