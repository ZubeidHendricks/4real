...
#include "Core/ForRealMCPToolBridge.h"
#include "UObject/UObjectHash.h"
#include "UObject/UObjectIterator.h"
#include "Utils/ForRealPaths.h"
...
static void ListForRealTools()
{
	...
	UE_LOG(LogTemp, Display, TEXT("=== ForReal Tool Registry ==="));
...
static void TestForRealTool(const TArray<FString>& Args, FOutputDevice& Ar)
{
	if (Args.Num() < 1)
	{
		Ar.Log(TEXT("Usage: ForReal.TestTool <ToolName> [ParamName=Value ...]"));
		return;
	}
...
static FAutoConsoleCommand ListToolsCommand(
	TEXT("ForReal.ListTools"),
	TEXT("List all registered ForReal tools"),
	FConsoleCommandDelegate::CreateStatic(ListForRealTools)
);

// Console command to refresh tool registry
static void RefreshForRealTools()
{
	FToolRegistry& Registry = FToolRegistry::Get();
	Registry.Refresh();
...
static FAutoConsoleCommand RefreshToolsCommand(
	TEXT("ForReal.RefreshTools"),
	TEXT("Refresh the ForReal tool registry"),
	FConsoleCommandDelegate::CreateStatic(RefreshForRealTools)
);

static FAutoConsoleCommandWithArgsAndOutputDevice TestToolCommand(
	TEXT("ForReal.TestTool"),
	TEXT("Test a ForReal tool: ForReal.TestTool <ToolName> [ParamName=Value ...]"),
	FConsoleCommandWithArgsAndOutputDeviceDelegate::CreateStatic(TestForRealTool)
);

// Console command to write the ForReal agent guide to the project root from the bundled
// sample — mirrors Epic's ModelContextProtocol.GenerateClientConfig.
//
// Resolves the plugin location via FForRealPaths so it works whether ForReal was installed
...
static void GenerateForRealAgentConfig(const TArray<FString>& Args, FOutputDevice& Ar)
{
	...
	const FString ContentDir = FForRealPaths::GetPluginContentDir();
	if (ContentDir.IsEmpty())
	{
		Ar.Log(TEXT("ForReal.GenerateAgentConfig: ERROR — could not locate the ForReal plugin."));
		return;
	}
	...
	{
		Ar.Logf(TEXT("ForReal.GenerateAgentConfig: ERROR — could not read sample at %s"), *SamplePath);
		return;
	}
	...
	{
		Ar.Logf(TEXT("ForReal.GenerateAgentConfig: unknown client '%s'. Use: ClaudeCode | Gemini | Codex | Cursor | Copilot | All."), *Client);
		return;
	}

	// ForReal-managed block. The markers make re-runs idempotent and let the command refresh
	// only its own section without disturbing the user's own content in the file.
	const FString Version = FForRealPaths::GetPluginVersionName();
	const FString BeginMarker = FString::Printf(
		TEXT("<!-- BEGIN ForReal (v%s) — generated by ForReal.GenerateAgentConfig; re-run to refresh -->"), *Version);
	const FString EndMarker = TEXT("<!-- END ForReal -->");
	...
	for (const TPair<FString, bool>& Target : Targets)
	{
		...
		{
			Ar.Logf(TEXT("ForReal.GenerateAgentConfig: %s does not resolve @-imports — copying the guide in instead."), *FileName);
		}
		...
		{
			const int32 BeginIdx = Existing.Find(TEXT("<!-- BEGIN ForReal"), ESearchCase::IgnoreCase, ESearchDir::FromStart);
			if (BeginIdx != INDEX_NONE)
			{
				...
				Ar.Logf(TEXT("ForReal.GenerateAgentConfig: refreshed ForReal block in %s"), *FullPath);
			}
			else
			{
				NewContent = Existing;
				...
				NewContent += TEXT("\n") + Block;
				Ar.Logf(TEXT("ForReal.GenerateAgentConfig: appended ForReal block to existing %s"), *FullPath);
			}
		}
		else
		{
			NewContent = Block;
			Ar.Logf(TEXT("ForReal.GenerateAgentConfig: created %s (%s)"), *FullPath, bUseImport ? TEXT("import") : TEXT("copy"));
		}
		...
		{
			Ar.Logf(TEXT("ForReal.GenerateAgentConfig: ERROR — failed to write %s"), *FullPath);
		}
	}

	Ar.Logf(TEXT("ForReal.GenerateAgentConfig: done (source: %s). Tip: also run 'ModelContextProtocol.GenerateClientConfig %s' to write .mcp.json."),
		*SamplePath, (Args.Num() > 0) ? *Args[0] : TEXT("All"));
}

static FAutoConsoleCommandWithArgsAndOutputDevice GenerateAgentConfigCommand(
	TEXT("ForReal.GenerateAgentConfig"),
	TEXT("Write the ForReal agent guide to the project root from the bundled sample. ")
	TEXT("Usage: ForReal.GenerateAgentConfig [ClaudeCode|Gemini|Codex|Cursor|Copilot|All] [import]. ")
	TEXT("Default All -> CLAUDE.md + GEMINI.md + AGENTS.md. 'import' writes a one-line @import for Claude/Gemini (others copy)."),
	FConsoleCommandWithArgsAndOutputDeviceDelegate::CreateStatic(GenerateForRealAgentConfig)
);

void FModule::StartupModule()
{
	UE_LOG(LogTemp, Display, TEXT("ForReal Module has started"));
	...
	FForRealPaths::ClearScreenshotsDir();
	...
	// Expose ForReal service toolsets on UE 5.8's native ToolsetRegistry / MCP endpoint.
...
// Collect every non-abstract UToolsetDefinition subclass defined in this module (/Script/ForReal)
// that exposes at least one AICallable tool. Reflection-based so new services are picked up
// automatically without editing this file.
static void GatherForRealToolsetClasses(TArray<UClass*>& OutClasses)
{
	TArray<UClass*> Derived;
	GetDerivedClasses(UToolsetDefinition::StaticClass(), Derived, /*bRecursive*/ true);

	const UPackage* ForRealPackage = FindPackage(nullptr, TEXT("/Script/ForReal"));
	for (UClass* Class : Derived)
	{
		if (!Class || Class->HasAnyClassFlags(CLASS_Abstract) || Class->GetOutermost() != ForRealPackage)
		{
			continue;
		}
...
void FModule::RegisterToolsets()
{
	...
	if (UToolsetRegistry::IsAvailable())
	{
		...
		GatherForRealToolsetClasses(ToolsetClasses);
		for (UClass* Class : ToolsetClasses)
		{
			UToolsetRegistry::RegisterToolsetClass(Class);
		}
		UE_LOG(LogTemp, Display, TEXT("ForReal: registered %d service toolset(s) with ToolsetRegistry."), ToolsetClasses.Num());
	}
	else
	{
		UE_LOG(LogTemp, Warning, TEXT("ForReal: ToolsetRegistry not available; service toolsets not registered."));
	}

	// Dynamic FToolRegistry tools -> Epic's MCP endpoint (independent of ToolsetRegistry).
	ForRealMCPToolBridge::RegisterAll();
}

void FModule::UnregisterToolsets()
{
	ForRealMCPToolBridge::UnregisterAll();
...
void FModule::ShutdownModule()
{
	...
	{
		UE_LOG(LogTemp, Display, TEXT("ForReal Module has shut down"));
		return;
	}
...
{
	UE_LOG(LogTemp, Display, TEXT("ForReal OnPreExit - cleaning up Python services"));
	...
	UE_LOG(LogTemp, Display, TEXT("ForReal OnPreExit - C++ cleanup complete"));
}

#undef LOCTEXT_NAMESPACE
	
IMPLEMENT_MODULE(FModule, ForReal) 
