...
namespace
{
	/** Map a ForReal FToolParameter type string to a JSON Schema type. */
	...
	/**
	 * Flatten an MCP arguments JSON object into the {name -> string} map ForReal tools expect.
	 * Scalars become their string form; nested objects/arrays are re-serialized to a JSON string
	 * (ForReal tools that take structured input expect it as a JSON string, e.g. "ParamsJson").
	 */
	...
	struct FForRealToolAdapter : IModelContextProtocolTool
	{
		explicit FForRealToolAdapter(const FToolMetadata& Meta)
			: Name(Meta.Name)
			...
			const FResultCallback& OnComplete) override
		{
			...
			auto Execute = [ToolName, Args = MoveTemp(Args), OnComplete]()
			{
				...
				// ForReal tools report failure as {"success": false, ...}; surface that as an MCP error.
				bool bIsError = false;
			...
			// ForReal tools must run on the game thread.
			if (IsInGameThread())
			{
				Execute();
			}
	...
	void RegisterAll()
	{
		...
		if (!Module)
		{
			UE_LOG(LogToolRegistry, Warning,
				TEXT("ForReal: ModelContextProtocol module not available; tools not exposed on MCP endpoint."));
			return;
		}
		...
		for (const FToolMetadata& Meta : FToolRegistry::Get().GetAllTools())
		{
			...
			TSharedRef<IModelContextProtocolTool> Tool = MakeShared<FForRealToolAdapter>(Meta);
			...
			{
				UE_LOG(LogToolRegistry, Warning,
					TEXT("ForReal: MCP tool '%s' was rejected (name collision or invalid name)."), *Meta.Name);
			}
		}

		UE_LOG(LogToolRegistry, Display,
			TEXT("ForReal: exposed %d tool(s) on Epic's MCP endpoint (%d internal/testing tools skipped)."),
			Registered, Skipped);
	}
