# 4real × Unreal Engine 5.8 — Toolset Comparison & Consolidation Plan

**Goal:** make 4real *enhance* Unreal 5.8's native AI toolset system, not compete with it. This
document inventories every Epic and 4real toolset/tool, maps the overlaps, and for each 4real
toolset says: **KEEP & ENHANCE**, **REFACTOR**, or **DELETE** (Epic already covers it).

Status as of the 5.8 migration: 4real is now an *extension* — 31 service toolsets + 10 bridged
MCP tools + 88 skills, all registered into Epic's `ToolsetRegistry` / `ModelContextProtocol` /
`AgentSkillToolset`. There is no separate 4real server, chat, or skill system anymore.
...
GAS, …). Much of 4real's surface now overlaps the engine's.

4real's durable value is in three areas Epic does **not** cover, plus depth Epic doesn't reach:
...
**Decision rule (default to cut):** a 4real toolset survives **only** if Epic has no equivalent, or
4real's depth is a *distinct, demonstrable user win Epic can't deliver*. "4real is a bit deeper"
...
> **Buckets below are source-verified** (§5) by reading each Epic toolset and the matching 4real
> service against each other in the engine + plugin source, with every domain toolset enabled live.

| Bucket | 4real toolsets/tools |
|---|---|
...
## 1.5 Target footprint — the lean 4real
...
deleting whole services, because 4real genuinely exceeds Epic almost everywhere. Grouped by user
...
- `StateTreeService` → **KEEP intact** — Epic's `StateTreeTools` is read-only; 4real owns all mutation/binding/compile
...
**Convention gap to close:** Epic passes/returns *typed structs* and *object references*; 4real's
...
| `AIAssistant.AIAssistantToolset` | `GetProjectContext`, `GetDockedContext` | In-editor assistant context (no 4real analog) |
| `ToolsetRegistry.AgentSkillToolset` | `ListSkills`, `GetSkills`, `CreateSkill`, `UpdateSkill` | **Skills** (4real packs now register here) |
| `UserDefinedEnum`, `UserDefinedStruct` | create/edit enum & struct | Overlaps 4real `EnumStructService` |

### Exists in engine, NOT enabled here (`Engine/Plugins/Experimental/Toolsets/`)

Enable these to compare exact tools before deleting the matching 4real service:
...
31 reparented service toolsets (`4real.<Name>Service`, ~1,052 AICallable tools) + 10 bridged MCP
tools. Method counts approximate.

| 4real toolset | ~tools | Epic counterpart? |
|---|---|---|
...
Verified by reading each Epic toolset and the matching 4real service in source, with all Epic domain
toolsets enabled live. **This table supersedes the narrative bullets below** where they differ.

| 4real service | Epic counterpart (live name) | Verdict | What survives / why |
|---|---|---|---|
...
| AnimSequence/AnimMontage/AnimGraphService | `animation_toolset.*` (Sequencer/ControlRig) | **KEEP** | Epic is Sequencer/ControlRig; 4real edits the anim *assets* — different |
| StateTreeService | `state_tree_toolset.StateTreeTools` | **KEEP** | **Epic is read-only inspection (~9 getters); 4real does all mutation/binding/compile** |
...
| DataTableService | `editor_toolset.DataTableTools` | **CUT** | Epic covers create/import/schema/rows CRUD; 4real deltas (GetInfo/RowExists/UpdateRow) marginal |
...
- **AnimSequenceService / AnimMontageService** — Epic's `AnimationAssistantToolset` is
  Sequencer/ControlRig-centric; 4real edits the *animation assets* (keyframes, bone tracks,
...
- **`execute_python_code` + `discover_python_*` + `list_python_subsystems`** — the Python escape
  ...
  4real's discovery trio has no native analog and is what makes the services self-teaching. *Keep;
...
- **Performance & Tracing** (currently the `editor_control` profiling actions) — `frame_timing`
  ...
  but cannot measure anything. This is one of 4real's strongest differentiators and is currently
  hidden inside an overloaded tool. → *Enhance:* extract into a dedicated `4real.PerformanceService`
...
1. **Typed structs over stringly-typed args.** Replace `FString`/`ParamsJson` inputs with USTRUCT
   ...
   rich `inputSchema` objects; 4real's JSON-string args produce opaque schemas. This mostly affects
...
4. **`AgentSkill` for guidance, tools for actions.** Already done — 4real packs are `UAgentSkill`s.
...
6. **Don't re-implement reflection.** Prefer `ObjectTools`/`AssetTools` for generic property/asset ops
   instead of bespoke 4real equivalents.
...
1. **Verify the "off" domain toolsets** — temporarily enable `GameplayTagsToolset`, `StateTreeToolset`,
   ...
   record exact deltas vs the matching 4real service.
...
4. **Extract the Performance/Tracing toolset** out of `editor_control` into its own
   `4real.PerformanceService` (frame_timing + Insights traces + analyse + standalone) — net-new
...
Net effect: 4real shrinks from 31 services + 10 tools to the **~18 toolsets** in §1.5 — only what
