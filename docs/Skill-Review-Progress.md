...
basics at the engine tool and keep only 4real's delta where the service was trimmed; **verify** if the
service was kept.
...
| asset-management | AssetDiscoveryService (TRIM) | REWRITE: engine AssetTools/Python for CRUD; 4real import/export only | ☑ | |
| level-actors | ActorService (TRIM) | REWRITE: engine ActorTools/SceneTools for ops; 4real lock/camera-framing | ☑ | |
| viewport | ViewportService (TRIM) | REWRITE: engine EditorAppToolset camera; 4real view-mode/FOV/exposure/layout | ☑ | |
| gameplay-tags | GameplayTagService (TRIM) | REWRITE: engine GameplayTagsToolset CRUD; 4real HasTag/AddTags/GetChildren/GetTagInfo | ☑ | |
...
| blueprints | BlueprintService (TRIM) | REWRITE: engine BlueprintTools for basic var/func/node; 4real higher-order | ☑ | |
| blueprint-graphs | BlueprintService (TRIM) | REWRITE: keep build_graph/timelines/etc; basic nodes → BlueprintTools | ☑ | |
| materials | MaterialService/Node (TRIM) | REWRITE: engine MaterialTools for basics; 4real lifecycle/export/HLSL | ☑ | |
| umg-widgets | WidgetService (TRIM) | REWRITE: engine UMGToolSet/MVVMToolset for CRUD; 4real font/brush/anim/preview | ☑ | |
| niagara-systems | NiagaraService (TRIM) | REWRITE: engine NiagaraToolsets for CRUD; 4real RI/diagnostics/scratch-pad | ☑ | |
| niagara-emitters | NiagaraEmitterService/ScratchPad (TRIM/KEEP) | REWRITE: engine for module/renderer; 4real color/curve + scratch-pad | ☑ | |
| skeleton | SkeletonService (TRIM) | REWRITE: engine SkeletalMeshTools for socket/inspect; 4real bone-edit/retarget/profiles | ☑ | |
| pie-testing | editor_control PIE (CUT) + WidgetService | REWRITE: engine EditorAppToolset PIE; 4real widget-in-PIE validation | ☑ | |
| profiling | editor_control profiling (CUT) | REWRITE → PerformanceService (`unreal.PerformanceService.*`) | ☑ | |
| 4real | entry point | REWRITE: new architecture (AgentSkillToolset, Python-first, no 4real server) | ☑ | |
...
**Rewritten — basics redirected to engine, 4real delta kept (17):** asset-management, level-actors,
viewport, gameplay-tags, engine-settings, project-settings, frame-rate, blueprints, blueprint-graphs,
materials, umg-widgets, niagara-systems, niagara-emitters, skeleton, pie-testing, profiling, 4real.
...
`4real-skills-manager` was replaced with `ListSkills`/`GetSkills` on `AgentSkillToolset`.
...
`4real-skills-manager` call site (→ engine tools or native Python); `editor_control` profiling →
`unreal.PerformanceService.*`; removed 4real-MCP-server / API-key language; fixed dangling
