...
|---|---|---|
| **Stage 0** — export terrain | `scripts/export_terrain_data.py` | inside Unreal (4real) |
...
# STAGE 0 — Export Terrain Data from 4real

You must already have a landscape generated/imported in your level with 4real (heightmap + paint/weight layers). This stage pulls that terrain out of the engine into the two host-readable inputs the designer consumes.
...
### STEPS
1. **In Unreal (via 4real), run `scripts/export_terrain_data.py`.** Set `LANDSCAPE` (or leave blank to auto-pick) and `OUT_DIR`, then execute it through `mcp__4real__execute_python_code` or *Tools ▸ Execute Python Script*. It uses `unreal.LandscapeService.export_weight_map()` for each paint layer and `export_heightmap()`, and writes `export_manifest.json` + `export_<Layer>.png` + `export_height.png`.
...
0. Export terrain from 4real → build `landcover_grid.json` (+ optional `river_world.json`) → gate
1. Roadways → gate
